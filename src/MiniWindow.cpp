...
void MiniWindow::Draw(BITMAP *destination, int x, int y)
{
	...
	{
		alfont_textout(destination, g_game->font12, "Bitmaps not established", x, y, al_map_rgb(255, 0, 255));
		return;
	}
	...
	for (int a = al_get_bitmap_height(mwCorner); a < mwHeight; a += al_get_bitmap_height(mwSide))
	{
		...
		{ 
			...
			float _angle_rad = ((float)(-64)) * ALLEGRO_PI * 2.0f / 256.0f; 
	...
	//upper left corner
	al_set_target_bitmap(buffer);
	al_draw_filled_rectangle(0, 0, al_get_bitmap_width(mwCorner)-1+1, al_get_bitmap_height(mwCorner)-1+1, al_map_rgb(255, 0, 255));
	al_set_target_bitmap(buffer); al_draw_bitmap(mwCorner, 0, 0, 0);
	
	//upper right corner
	al_set_target_bitmap(buffer);
	al_draw_filled_rectangle(mwWidth - al_get_bitmap_width(mwCorner), 0, mwWidth-1+1, al_get_bitmap_height(mwCorner)-1+1, al_map_rgb(255, 0, 255));
	...
	//lower left corner
	al_set_target_bitmap(buffer);
	al_draw_filled_rectangle(0, mwHeight - al_get_bitmap_width(mwCorner), al_get_bitmap_height(mwCorner)-1+1, mwHeight-1+1, al_map_rgb(255, 0, 255));
	...
	//lower right corner
	al_set_target_bitmap(buffer);
	al_draw_filled_rectangle(mwWidth - al_get_bitmap_width(mwCorner), 0 + mwHeight - al_get_bitmap_height(mwCorner), mwWidth-1+1, mwHeight-1+1, al_map_rgb(255, 0, 255));
