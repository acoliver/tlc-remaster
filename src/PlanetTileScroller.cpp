...
PlanetTileScroller::PlanetTileScroller(int TilesAcross, int TilesDown, int TileWidth, int TileHeight):
   scrollbuffer(0),
   tiles(0),
   tileWidth(TileWidth),
...
PlanetTileScroller::~PlanetTileScroller()
{
    this->Destroy();
}

void PlanetTileScroller::Destroy()
{
   if (scrollbuffer) 
   {
...
void PlanetTileScroller::ResetTiles() 
{
...
void PlanetTileScroller::ResetPointData() 
{
...
void PlanetTileScroller::ClearTileImageCache()
{
...
void PlanetTileScroller::PurgePrimaryImages()
{
...
bool PlanetTileScroller::LoadTileSet(BITMAP *tileImage, int Variations, bool GroundNavigation, bool AirNavigation)
{
	if (!tileImage) {
...
bool PlanetTileScroller::GenerateTiles()
{
...
BITMAP *PlanetTileScroller::GenerateTile(int BaseTileSet, int TileX, int TileY)
{
	int key = 0;
...
short PlanetTileScroller::CalcTileBaseType(int TileX, int TileY)
{
	int pdIndexes[4];
	pdIndexes[0] = pdIndex(TileX, TileY);
	pdIndexes[1] = pdIndex(TileX, TileY + 1);
...
BITMAP *PlanetTileScroller::FindTile(int key)
{
...
int PlanetTileScroller::CreateScrollBuffer(int Width, int Height)
{
	windowWidth = Width;
	windowHeight = Height;
...
**/ 
void PlanetTileScroller::DrawScrollWindow(BITMAP *Dest, int X, int Y, int Width, int Height)
{
	//prevent a crash
	if (tileWidth < 1 || tileHeight < 1) return;
...
bool PlanetTileScroller::CheckCollisionbyCoords(int X, int Y, bool Flying)
{
	bool result = false;
...
void PlanetTileScroller::ConvertCoordstoNearestPoint(int &X, int &Y)
{
	double x, y = 0;
